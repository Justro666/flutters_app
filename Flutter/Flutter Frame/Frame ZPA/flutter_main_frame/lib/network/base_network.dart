...
class BaseNetwork {
  Future<ResponseOb> getRequest({String endUrl, bool isWallet = false}) async {
    ...
    return http.get(Uri.https(BASE_URL + endUrl)).then((res) {
  ...
  Future<ResponseOb> postRequest(
      {String endUrl, Map body, bool isWallet = false}) async {
    ResponseOb rv = ResponseOb();
    return http.post(Uri.https(BASE_URL + endUrl), body: body).then((res) {
      if (res.statusCode == 200) {
        rv.message = MsgState.data;
        rv.data = res.body;
      } else {
